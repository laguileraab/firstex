...
public class RestAPIProduct {
    ...
    public ResponseEntity<?> addProduct(@RequestBody Product productRequest) {
        new Product();
        Product product = new Product(productRequest.getSize(),productRequest.getColor(),productRequest.getPrice(),productRequest.getFragile(), product.getEnvelop(), product.getLot());
