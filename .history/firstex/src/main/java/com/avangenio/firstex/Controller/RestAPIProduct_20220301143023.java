...
public class RestAPIProduct {
    ...
    public ResponseEntity<?> addProducts(@RequestBody Set<Product> products, @PathVariable int id) {
        try {
            ...
            products.forEach((product) -> {
                ...
                if (isInEnum(product.getEnvelop(), EnvelopType.class)) {
                    not_found = true;
                }
            });
            if (not_found) {
    ...
    private boolean isInEnum(EnvelopType envelop, Class<EnvelopType> enumClass) {
        return Arrays.stream(enumClass.getEnumConstants()).anyMatch(e -> e.name().equals(envelop.toString()));
    }
