    ...
    public ResponseEntity<?> addProducts(@RequestBody Set<Product> products, @PathVariable int id) {
        try {
            ...
            products.forEach((product) -> {
                ...
                Arrays.stream(EnvelopType.getEnumConstants()).anyMatch(product - >product.getEnvelop().equals(dayOfWeek))
                                if (!Object.values(EnvelopType).includes(product.getEnvelop())) {
